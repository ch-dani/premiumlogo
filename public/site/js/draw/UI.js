...
$('.open_preview_modal').click(async e => {
    ...
    const backgroundElement = edit.canvas.getObjects().find(o => o._element_type == 'background');

    let b64 = '';
    if (clipElement) {
        const bgOpacity = backgroundElement.opacity;
        clipElement.set({ opacity: 0 });
        backgroundElement.set({ opacity: 0 });

        const multiplier = Math.min(
            500 / clipElement.width * clipElement.scaleX,
            500 / clipElement.height * clipElement.scaleY
        );
        ...
        clipElement.set({ opacity: 1 });
        backgroundElement.set({ opacity: bgOpacity });
    } else {
...
$('.template_buttons_edit').on('click', e => {
    if (templateEditor && templateEditor.categories) {
        const key = e.delegateTarget.dataset.action;
        const data = templateEditor.categories[key];
        ...
        $('#open_templates').find('.popup_name').html(title);

        let html = '';
        for (const item of data) {
            html += `<div class="templates_item" data-id="${item.id}">
            <div class="templates_image">
                <img src="${item.thumbnail}"/>
            </div>

            <div class="templates_name">${item.name}</div>
        </div>`;
        }

        $('#open_templates').find('.content_popup_row').html(html);
        $('#open_templates').find('.templates_item').on('click', async e => {
            await templateEditor.preloadTeplate(e.delegateTarget.dataset.id);
        });
    }
});
...
(async function () {
    ...
    document.getElementsByClassName('btn_save')[0].addEventListener('click', async function (e) {
        e.preventDefault();
        showLoader("body");
        const rect = edit.canvas.getObjects().find(o => o._element_type == 'clip');

        let svgLogo = '';
        let original = '';
        if(rect) {
            svgLogo = await edit.exportSVGByClip();
            original = await edit.exportSVGByClip(true, true);
        } else {
            svgLogo = await edit.getCanvasDataURL({ _fonts, padding: 20, type: 'svg' });
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                logo: svgLogo, ...(original ? { original } : {}), 
                category: window.templateCategory
            },
