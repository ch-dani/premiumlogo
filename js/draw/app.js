...
class Util {
    constructor() {
        this.defaultProperties = ['transparentCorners', '_id', 'fill'];
    }
...
class LogoEditor {
    ...
    parseSVGBeforeDownload(svg, fontData = []) {
        //        console.log(1, svg);
    ...
    async loadImageByURL(url, prefix, active_by_default=true) {
        if (!url) return;
        if (/\.svg+$/.test(url)) {
            ...
            return await new Promise(resolve => {
                fabric.Image.fromURL(url, img => {
                    ...
                    img.set(this.groupToCenter(img));
    ...
    async getCanvasDataURL(opts = {}) {
        ...
        if (opts.type == 'svg') {
            ...
            svg = this.parseSVGBeforeDownload(svg, opts._fonts || []);
